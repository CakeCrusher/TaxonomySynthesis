...
from openai.types.chat.chat_completion_tool_message_param import ChatCompletionToolMessageParam  # noqa
...
```"""  # noqa
    ...
    def generate_categories(
        ...
        if not response.choices or not response.choices[0].message.tool_calls:
            raise ValueError("No tool calls found in the response from the model.")  # noqa
        ...
        if tool_call is None or tool_call.function.arguments is None:
            raise ValueError("Tool call arguments are missing in the model response.")  # noqa
